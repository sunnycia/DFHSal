...
def preprocess_images(paths, shape_r, shape_c):
    ...
    ## uncomment this if using theano, or dataformat is BCHW
...
def postprocess_predictions(pred, shape_r, shape_c):
    ...
    pred = cv2.resize(pred, (shape_c, shape_r))
    img = pred
...
def generator(img_dir, dens_dir, shape_r=224, shape_c=224, b_s=8):
    images = [img_dir + f for f in os.listdir(img_dir) if f.endswith(('.jpg', '.jpeg', '.png'))]
    maps = [dens_dir + f for f in os.listdir(dens_dir) if f.endswith(('.jpg', '.jpeg', '.png'))]
    ...
    while True:
      ...
      Y = preprocess_maps(maps[counter:counter+b_s], shape_r, shape_c)
